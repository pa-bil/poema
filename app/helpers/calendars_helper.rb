module CalendarsHelper
  def calendars_navi_path(calendar, last_as_link = false)
    n = []
    n << link_to("Wydarzenia", calendars_path)
    n << (last_as_link ? link_to(calendar.title, calendar_path(calendar)) : h(calendar.title))
    n
  end
  ...
  #
  # http://github.com/adzap/calendar_helper 
  #
  ...
  def calendar(options = {}, &block)
    ...
    options[:month_name_text] = I18n.localize(DateTime.new(options[:year], options[:month]), {:format => "%B"}) + ' ' + options[:year].to_s
    ...
    day_names = ['Nd', 'Po', 'Wt', 'Sr', 'Cz', 'Pt', 'So']
...
#    end
    ...
    beginning_of_week(first, first_weekday).upto(first - 1) do |d|
      cal << %(<td )
      cal << "id='" + d.strftime("#{options[:id_prefix]}_%Y_%m_%d") + "'"
    ...
    first.upto(last) do |cur|
      ...
      cal << "<td id='" + cur.strftime("#{options[:id_prefix]}_%Y_%m_%d") + "' #{cell_attrs}>#{cell_text}</td>"
